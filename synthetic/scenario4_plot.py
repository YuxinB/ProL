...
def main():
    ...
    plt.plot(times, np.mean(all_erm, axis=1), label="ERM", alpha=0.7)
    plt.plot(times, np.mean(all_pr, axis=1), label="Prospective", alpha=0.7)
    bayes_err = 0.357
    plt.plot(times, np.ones_like(times) * bayes_err, '--', label='Bayes Optimal', color='black')

    plt.scatter(times, np.mean(all_erm, axis=1), s=15, alpha=1.0)
    plt.scatter(times, np.mean(all_pr, axis=1), s=15, alpha=1.0)
